...
CMD=$1
XZCAT=/usr/bin/xzcat
BUILDDIR="/home/flaris/git/MobilityDB/build"
WORKDIR=${BUILDDIR}/tmptest
EXTFILE="/home/flaris/git/MobilityDB/build/test_mobilitydb--1.0.0.sql"
...
pg_status() {
  /usr/lib/postgresql/14/bin//pg_ctl -D "${DBDIR}" status
}

pg_stop() {
  /usr/lib/postgresql/14/bin//pg_ctl -D "${DBDIR}" stop
}
...
POSTGIS="/usr/lib/postgresql/14/lib/postgis-3.so"
...
create_ext)
...

  {
    ...
    echo "CREATE EXTENSION postgis WITH VERSION '3.3.1';" | $PSQL 2>&1
